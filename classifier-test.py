...
#network
# classifier = FCNET(3*2**16, [20], 1).to(device)
classifier = ConvNet([16,16,16],include_dense=True).to(device)
print(classifier)
...
if train:
	for epoch in range(num_epochs):
		...
		for i,batch in enumerate(trainLoader):
			...
			train_images, train_labels = batch[0].to(device), batch[1].to(device)
...
with torch.no_grad():
	classifier.eval()
	if not train:
		classifier.load_state_dict(torch.load('D:/ML/GANEvaluation/model-files/2Conv1FC-161620/73-61.50.pth'))
